/*
  ...
  $Id: jfn_rf_files2.c,v 1.1 2018/08/10 17:21:32 jfnielse Exp $
 */
...
int jfn_rf_getfilename(char *fname, int index, const char* directory) {
	...
	FILE *fid;
  	...
  	sprintf(tmpfname, "%68srffiles.txt", directory);
...
int jfn_rf_readheader(char *fname, rfstruct *rfinfo)
{
	...
	fprintf(stderr,"jfn_rf_readheader(): reading %s \n", fname);
        if (sizeof( rfinfo->fname ) <= strlen( fname ) ) {
            fprintf(stderr,"jfn_rf_readheader(): file name too long\n");
            return(JFN_FAILURE);
        }
	strcpy(rfinfo->fname, fname);

	if ((fid = fopen (fname, "r")) == NULL) {
            fprintf(stderr,"jfn_rf_readheader(): fopen failed\n");
		return(JFN_FAILURE);
        }
...
*/
	...
	readshort(&(rfinfo->nparamsint16), 1, fid);
        if (sizeof( rfinfo->paramsint16 )/sizeof( rfinfo->paramsint16[0] )  < rfinfo->nparamsint16 ) {
            fprintf(stderr,"jfn_rf_readheader(): nparamsint16 to high\n");
            return(JFN_FAILURE);
        };
...
*/
	...
	readshort(&(rfinfo->nparamsfloat), 1, fid);
        if (sizeof( rfinfo->paramsfloat )/sizeof( rfinfo->paramsfloat[0] ) < rfinfo->nparamsfloat ) {
            fprintf(stderr,"jfn_rf_readheader(): nparamsfloat to high\n");
            return(JFN_FAILURE);
        };
...
{
    /* NOTE: of rfinfo the following fields need to be set before calling this function: (these and more are filled by jfn_rf_readheader)
     *   fname
     *   dataoffset
     *   npulses
     *   ncoils
     *   res
     * The following arrays need to be allocated, but are filled in this function:
     *   rho[ npulses ][ ncoils ][ res ]
     *   theta[ npulses ][ ncoils ][ res ]
     *   gx, gy, gz : [npulses][ res ]
    */
	FILE *fid;
