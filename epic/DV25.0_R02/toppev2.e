/*@Start***********************************************************
 ...
 7-2018, D.Poot, Erasmus MC: added checks and 'epic_error' calls in predownload.
                             fixed waitcores allocation bug.
 
 $Id: toppev2.e,v 1.1 2018/08/10 17:21:32 jfnielse Exp $
*@End*************************************************************/
...
#define LOOPARRLENGTH 9000000
int looparr[ LOOPARRLENGTH ];    /* scan loop definition, loaded from scanloop.txt */
...
int cvinit()
{
	...
	rcvn_flag = 2;   /* Necessary change around DV23 or so. rcvn_flag is declared in Prescan.e. JFN Dec 2015 */
...
int cveval()
{
  ...
  if ( jfn_rf_readheader("tipdown.mod", &rfdinfo)==JFN_FAILURE) {
      epic_error(0, "jfn_rf_readheader() failed when reading tipdown.mod.",0,0);
      return FAILURE;
  }
  ...
  /* If using external .mod RF pulses, don't allow opflip to be larger than nominal (design) flip angle */
  if (rftype == 0 && opflip > rfpulse[RFD_SLOT].nom_fa) {
    temp = _opflip.fixedflag;
   _opflip.fixedflag = 0;
...
int predownload()
{
	...
	rfstruct roinfotmp;            /* temporary to check if all .mod files load correctly */
...
*/
	...
	maxecho   = loophdr[2];
	maxview   = loophdr[3];
	fprintf(stderr, "predownload(): nstartseq = %d, maxslice = %d \n", nstartseq, maxslice);
	...
	for ( j=0; j<ncores; j++) {
		if ( jfn_rf_readheader(coredefinfo.wavfiles[j], &roinfotmp ) == JFN_FAILURE ) {
			epic_error(use_ermes, "jfn_rf_readheader could not load core from file %s.",EM_PSD_SUPPORT_FAILURE, EE_ARGS(1), STRING_ARG, coredefinfo.wavfiles[j] );
			return FAILURE;
		}    
		/* TODO: check waveforms. Do gradient waveforms satisfy slew constraints and start and end at zero. Do compute SAR of RF waweforms.*/
	}
	for ( j=0; j<ncores; j++) {
...
/*#define fprintf_DEBUGPULSEGEN if (0) fprintf /* to not show debug prints (but check syntax). Assumes compiler optimizes dead branch away. */
#define fprintf_DEBUGPULSEGEN fprintf  /* to show debug prints. */
...
WF_PULSE jfn_rf_makepulse(WF_PROCESSOR wfp, char *pname, rfstruct *rfinfo, int tbeg)
{
   ...
   switch (wfp) {
      ...
      case TYPTHETA:
         ...
         break;
/*      default :
         epic_error(0, "jfn_rf_makepulse() failed. Invalid wave type specified,",0,0);
         break;*/
...
WF_PULSE jfn_wait(WF_PROCESSOR wfp, char *pname, int tbeg)
{
   ...
   WF_PULSE proto = INITPULSE;
   int res = 1;
   fprintf_DEBUGPULSEGEN(stderr, ", AllocNode");
   ep = (WF_PULSE *) AllocNode(sizeof(WF_PULSE));
   fprintf_DEBUGPULSEGEN(stderr, ", memcpy");
   memcpy((char*) ep, (char*) &proto, sizeof(WF_PULSE));
   fprintf_DEBUGPULSEGEN(stderr, ", pulsename");
   pulsename(ep, pname);
   fprintf_DEBUGPULSEGEN(stderr, ", createreserve");
   createreserve(ep, wfp, res);

   fprintf_DEBUGPULSEGEN(stderr, ", createconst");
   createconst(ep, wfp, 4*res, 0); 
   fprintf_DEBUGPULSEGEN(stderr, ", createinstr");
   createinstr(ep, tbeg, 4*res, 0);
   fprintf_DEBUGPULSEGEN(stderr, ", return");
...
STATUS pulsegen(void)
{
	...
	ncores = coredefinfo.ncores;
	fprintf(stderr, "\tpulsegen(): ncores = %d \n", ncores);
	...
	gxwav  = (WF_PULSE **) AllocNode(ncores*sizeof(WF_PULSE *));/* TODO?: not matched with FreeNode */
	gywav  = (WF_PULSE **) AllocNode(ncores*sizeof(WF_PULSE *));/* TODO?: not matched with FreeNode */
	gzwav  = (WF_PULSE **) AllocNode(ncores*sizeof(WF_PULSE *));/* TODO?: not matched with FreeNode */
	rhowav    = (WF_PULSE **) AllocNode(ncores*sizeof(WF_PULSE *)); /* TODO?: not matched with FreeNode */
	thetawav  = (WF_PULSE **) AllocNode(ncores*sizeof(WF_PULSE *));/* TODO?: not matched with FreeNode */
	gxwavp  = (WF_HW_WAVEFORM_PTR **) AllocNode(ncores*sizeof(WF_HW_WAVEFORM_PTR *));/* TODO?: not matched with FreeNode */
	gywavp  = (WF_HW_WAVEFORM_PTR **) AllocNode(ncores*sizeof(WF_HW_WAVEFORM_PTR *));/* TODO?: not matched with FreeNode */
	gzwavp  = (WF_HW_WAVEFORM_PTR **) AllocNode(ncores*sizeof(WF_HW_WAVEFORM_PTR *));/* TODO?: not matched with FreeNode */
	rhowavp = (WF_HW_WAVEFORM_PTR **) AllocNode(ncores*sizeof(WF_HW_WAVEFORM_PTR *));/* TODO?: not matched with FreeNode */
	thetawavp  = (WF_HW_WAVEFORM_PTR **) AllocNode(ncores*sizeof(WF_HW_WAVEFORM_PTR *));/* TODO?: not matched with FreeNode */

	for ( j=0; j<ncores; j++ ) {

		/* read and load .mod file for core j */
		jfn_rf_readheader(coredefinfo.wavfiles[j], &coresinfo[j]);
		fprintf_DEBUGPULSEGEN(stderr, "alloc");
		jfn_rf_allocatemem(&coresinfo[j]);     
		fprintf_DEBUGPULSEGEN(stderr, ", read waveforms");
		jfn_rf_readwaveforms(&coresinfo[j], ishard);

		/* Gradient waveforms */
		sprintf(gxname, "gxcore%d", j);
		fprintf_DEBUGPULSEGEN(stderr, ", make %s", gxname);
		gxcores[j] = jfn_rf_makepulse(TYPXGRAD, gxname, &coresinfo[j], RUP_GRD(start_core)); /* TODO */
		sprintf(gyname, "gycore%d", j);
		fprintf_DEBUGPULSEGEN(stderr, ", make %s", gyname);
		gycores[j] = jfn_rf_makepulse(TYPYGRAD, gyname, &coresinfo[j], RUP_GRD(start_core));
		sprintf(gzname, "gzcore%d", j);
		fprintf_DEBUGPULSEGEN(stderr, ", make %s", gzname);
		...
		if (hasDAQ[j] == 1) {
			...
			fprintf_DEBUGPULSEGEN(stderr, ", make %s", name);
		...
		fprintf_DEBUGPULSEGEN(stderr, ", make pointers\n");
		/* create pointer to all waveforms in this module (core) */
		gxwav[j] = (WF_PULSE *) AllocNode(coresinfo[j].npulses*sizeof(WF_PULSE));/* TODO?: not matched with FreeNode */
		gywav[j] = (WF_PULSE *) AllocNode(coresinfo[j].npulses*sizeof(WF_PULSE));/* TODO?: not matched with FreeNode */
		gzwav[j] = (WF_PULSE *) AllocNode(coresinfo[j].npulses*sizeof(WF_PULSE));/* TODO?: not matched with FreeNode */
		gxwavp[j] = (WF_HW_WAVEFORM_PTR *) AllocNode(coresinfo[j].npulses*sizeof(WF_HW_WAVEFORM_PTR));/* TODO?: not matched with FreeNode */
		gywavp[j] = (WF_HW_WAVEFORM_PTR *) AllocNode(coresinfo[j].npulses*sizeof(WF_HW_WAVEFORM_PTR));/* TODO?: not matched with FreeNode */
		gzwavp[j] = (WF_HW_WAVEFORM_PTR *) AllocNode(coresinfo[j].npulses*sizeof(WF_HW_WAVEFORM_PTR));/* TODO?: not matched with FreeNode */
		if (hasRF[j] == 1) {
			rhowav[j] = (WF_PULSE *) AllocNode(coresinfo[j].npulses*sizeof(WF_PULSE));/* TODO?: not matched with FreeNode */
			thetawav[j]  = (WF_PULSE *) AllocNode(coresinfo[j].npulses*sizeof(WF_PULSE));/* TODO?: not matched with FreeNode */v
			rhowavp[j] = (WF_HW_WAVEFORM_PTR *) AllocNode(coresinfo[j].npulses*sizeof(WF_HW_WAVEFORM_PTR));/* TODO?: not matched with FreeNode */
			thetawavp[j]  = (WF_HW_WAVEFORM_PTR *) AllocNode(coresinfo[j].npulses*sizeof(WF_HW_WAVEFORM_PTR));/* TODO?: not matched with FreeNode */
		}
		for (i = 0; i < coresinfo[j].npulses; i++) {
			sprintf(tstr, "gxwav_%d_%d", j, i);
			fprintf_DEBUGPULSEGEN(stderr, ", make %s", tstr);
		...
		fprintf_DEBUGPULSEGEN(stderr, ",\nmake %s", waitname);
		waitcores[j] = jfn_wait(SSP, waitname, RUP_GRD(mindur+4));
		fprintf_DEBUGPULSEGEN(stderr, "jfn_wait finished");
		...
		fprintf_DEBUGPULSEGEN(stderr, ", createseq %s\n", name);
...
*/
	...
	/* looparr specification: implicit 2D array, for core j use indices looparr[ j*NL .. j*NL + NL-1 ]
	* meaning of looparr[ j*NL + offset ] :
	* Offset  : meaning
	*     0   : core index, one based counting. 
	*     1   : setiamp rho l
	*    11   : setiphase rho
	*    14   : setfrequency rho; looparr[j*NL+14]/TARDIS_FREQ_RES
	*     2   : setiamp theta
	*     3   : setiamp gxcore
	*     4   : setiamp gycore
	*     5   : setiamp gzcore
	*    15   : ? using in indexing for setwave call:  gxwavp[icore][ looparr[ .. 15] -1 ]
	*     9   : boolean: true = DABON, false = DABOFF
	*     6   : loaddab slice
	*     7   : loaddab echo
	*     8   : loaddab view
	*    12   : setiphase on echocores (probably related to dab phase)
	*    10   : rotation of xy gradients; with phase (in rad) =  looparr[j*NL+10]/max_pg_iamp*M_PI
	*    13   : setperiod : set wait pulse duration  
	*/
