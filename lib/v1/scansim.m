function [rho, th, gx, gy, gz, rho1, th1, gx1, gy1, gz1, textra] = scansim(nstart, nstop, d, TPARAMS, dodisplay)
% function [rho, th, gx, gy, gz] = scansim(nstart, nstop, [d, TPARAMS, dodisplay])
%
...
for it = nstart:nstop
	...
	rho1 = [zeros(round((start_core+coredel)/dt),1); ia_rf/max_pg_iamp*cores{ic}.rho; zeros(round((timetrwait+timessi)/dt),1)];
	th1  = [zeros(round((start_core+coredel)/dt),1); ia_th/max_pg_iamp*cores{ic}.th;  zeros(round((timetrwait+timessi)/dt),1)];
	...
	gz1  = [zeros(round((start_core)/dt),1);         ia_gz/max_pg_iamp*cores{ic}.gz;  zeros(round((timetrwait+timessi+coredel)/dt),1)];
...
if dodisplay
	...
	gmax = 5;  % Gauss/cm
	subplot(511); plot(T, rho); ylabel('rho');   axis([T(1) 1.01*T(end) -1.1*max(abs(rho)) 1.1*max(abs(rho))]);
